...
from ollama import ResponseError  # if Ollama's client defines a ResponseError exception
...
def check_connection(client: Client) -> bool:
    try:
        client.list()
        return True
    except (ConnectError, TimeoutException, ResponseError) as e:
        # These are transient errors we want to retry
        logger.warning(f"Ollama connection failed: {e}. Retrying...")
        raise  # propagate to trigger tenacity retry
    except Exception as e:
        # Any other exception – don't retry, just log and signal failure
        logger.error(f"Failed to connect to Ollama: {type(e).__name__}: {e}")
        return False
