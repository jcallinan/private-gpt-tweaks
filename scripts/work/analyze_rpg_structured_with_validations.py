...
# === STRUCTURED FORMAT WITH VALIDATION FOCUS ===
STRUCTURED_FORMAT = """
...
List **all** input field validations found in the RPG code:
- Required fields
- Allowed values
- Error codes/messages
- Conditional checks (e.g. if A = X, then B must not be blank)
- Where in the program this occurs (if available)
...
def build_prompt(rpg_code):
    return f"""
You are analyzing legacy IBM RPG code to extract a **structured business use case**, with a special focus on **input type validation logic**.
...
Use the exact format below. Fill out all sections. Pay special attention to the **"Input Type Validation Checks"** — extract **every possible validation** you can identify.
...
def is_structured_output(text):
    required_sections = ["## Identification", "## Description", "## Process Steps", "## Input Type Validation Checks"]
...
def extract_validation_block(text):
    ...
    for line in text.splitlines():
        if line.strip().lower().startswith("## input type validation checks"):
            in_block = True
            continue
...
def process_chunk(args):
    ...
    if result:
        if is_structured_output(result):
            validations = extract_validation_block(result)
            if not validations or len(validations.splitlines()) <= 1:
                fail_path = os.path.join(LOG_DIR, f"no_validations_chunk_{index+1:02d}.txt")
                with open(fail_path, "w") as f:
                    f.write(result)
                print(f"⚠️ Chunk {index+1}: Output missing validations — logged to {fail_path}")
                return None
...
if __name__ == "__main__":
    ...
    print(f"🔍 Extracting structured use cases with validations from {len(args)} chunks using {cpu_count()} CPUs...")
