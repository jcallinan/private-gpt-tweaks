...
def normalize_headers(text):
    ...
    # Patch Use Case ID to format like UC-AP-AP160-002
    pattern = r"(\*\*Use Case ID\*\*:\s*UC-AP-)(\d+)"
    text = re.sub(pattern, lambda m: f"{m.group(1)}AP{PROGRAM_NAME}-{m.group(2)}", text)
...
def extract_title(text, program):
    match = re.search(r"Use Case ID.*?UC-[^\n]*", text, re.IGNORECASE)
    id_part = match.group(0).strip().split()[-1].replace("UC-", "") if match else "XXX"
    match2 = re.search(r"(?i)Use Case.*?:\s*(.*?)\n", text)
    title = re.sub(r'[^a-zA-Z0-9]+', '-', match2.group(1).strip()) if match2 else "Untitled"
    return f"UC-{MODULE_NAME}-{program}-{id_part}-{title}"[:80]
...
def process_chunk(i, chunk, template, ap200_context, outdir, logdir, rawfile, docx_dir, program):
    ...
    if result:
        ...
        if is_valid_output(result):
            ...
            with open(os.path.join(outdir, f"{filename_base}.md"), "w") as f:
                f.write(result + "\n")
            save_as_docx(result, os.path.join(docx_dir, f"{filename_base}.docx"))
...
if __name__ == "__main__":
    ...
    outdir = os.path.join(OUTPUT_BASE, f"mistral_debug_ap{PROGRAM_NAME}_{ts}")
