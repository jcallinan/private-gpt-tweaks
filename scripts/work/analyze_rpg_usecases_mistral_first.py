...
STRUCTURED_FORMAT = """<same as before>"""  # keep your formatted string here for brevity
...
def build_prompt(rpg_code):
    return f"""
You are analyzing IBM RPG code to extract a structured business use case.
...
def process_chunk(chunk_index, chunk, template_lines, output_dir):
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(OUTPUT_BASE, f"mistral_run_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(LOG_DIR, exist_ok=True)

    print(f"🚀 Starting Mistral-first analysis of {len(merged_chunks)} chunks...\n")

    all_results = []
    for i, chunk in enumerate(merged_chunks):
        result = process_chunk(i, chunk, template_lines, output_dir)
        if result:
